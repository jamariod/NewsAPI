...
//  NewsAPI
//
//  Created by Jamario Davis on 1/4/21.
//
...
final class HeaderView: UIView {
    ...
    private lazy var headerNewsPaperImage: UIImageView = {
        let v = UIImageView()
        v.translatesAutoresizingMaskIntoConstraints = false
        v.contentMode = .scaleAspectFit
        let config = UIImage.SymbolConfiguration(pointSize: fontSize, weight: .bold)
        v.image = UIImage(systemName: "newspaper", withConfiguration: config)?.withTintColor(.red, renderingMode: .alwaysOriginal)
        return v
    }()
    ...
    private lazy var headerStackView: UIStackView = {
        let v = UIStackView(arrangedSubviews: [headerNewsPaperImage, headingLabel, plusImage])
        v.translatesAutoresizingMaskIntoConstraints = false
        v.axis = .horizontal
        return v
    }()
